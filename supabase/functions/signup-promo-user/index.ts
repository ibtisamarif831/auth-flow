...
serve(async (req: Request) => {
  ...
  try {
    ...
    // update user referrer promo 
    await supabaseAdmin.from('users').update({ referrer_promo }).eq('id', userId)
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/signup-promo-user' \
